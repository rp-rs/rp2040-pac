...
pub type DMACH_W<'a, REG, const O: u8> = crate::FieldWriter<'a, REG, 4, O>;
...
pub type CALC_W<'a, REG, const O: u8> = crate::FieldWriter<'a, REG, 4, O, CALC_A>;
impl<'a, REG, const O: u8> CALC_W<'a, REG, O>
where
...
pub type OUT_REV_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
...
pub type OUT_INV_W<'a, REG, const O: u8> = crate::BitWriter<'a, REG, O>;
impl R {
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn en(&mut self) -> EN_W<SNIFF_CTRL_SPEC, 0> {
        EN_W::new(self)
    }
    #[doc = "Bits 1:4 - DMA channel for Sniffer to observe"]
    #[inline(always)]
    #[must_use]
    pub fn dmach(&mut self) -> DMACH_W<SNIFF_CTRL_SPEC, 1> {
        DMACH_W::new(self)
    }
    #[doc = "Bits 5:8"]
    #[inline(always)]
    #[must_use]
    pub fn calc(&mut self) -> CALC_W<SNIFF_CTRL_SPEC, 5> {
        CALC_W::new(self)
    }
    #[doc = "Bit 9 - Locally perform a byte reverse on the sniffed data, before feeding into checksum.  
 ...
 Note that the sniff hardware is downstream of the DMA channel byteswap performed in the read master: if channel CTRL_BSWAP and SNIFF_CTRL_BSWAP are both enabled, their effects cancel from the sniffer's point of view."]
    #[inline(always)]
    #[must_use]
    pub fn bswap(&mut self) -> BSWAP_W<SNIFF_CTRL_SPEC, 9> {
        BSWAP_W::new(self)
    }
    #[doc = "Bit 10 - If set, the result appears bit-reversed when read. This does not affect the way the checksum is calculated; the result is transformed on-the-fly between the result register and the bus."]
    #[inline(always)]
    #[must_use]
    pub fn out_rev(&mut self) -> OUT_REV_W<SNIFF_CTRL_SPEC, 10> {
        OUT_REV_W::new(self)
    }
    #[doc = "Bit 11 - If set, the result appears inverted (bitwise complement) when read. This does not affect the way the checksum is calculated; the result is transformed on-the-fly between the result register and the bus."]
    #[inline(always)]
    #[must_use]
    pub fn out_inv(&mut self) -> OUT_INV_W<SNIFF_CTRL_SPEC, 11> {
        OUT_INV_W::new(self)
    }
